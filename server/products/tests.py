...
class ProductListAPIViewTestCase(APITestCase):
    ...
    def test_list_visible_products(self):
        """List the products"""
        response = self.client.get(self.url)
        self.assertEqual(200, response.status_code)

        # must show only 4 products once 1 is not visible
        self.assertEqual(4, len(response.data))

    def test_product_creation_by_superuser(self):
        """Create a new product with superuser credentials"""
        self.client.login(username='admin', password='admin123')
        data = {'title': 'Product 6', 'description': 'Description 6', 'price': 3444.90}
    ...
    def test_product_creation_by_regular_user(self):
        ...
        data = {'title': 'Product 7', 'description': 'Description 7', 'price': 34.90}
        response = self.client.post(self.url, data)
